...
### 引用计数
引用计数是一种常见的场景，如果当前变量没用使用者是就可以释放，我们并不关心实际引用计数值是多少，相反我们只关心当前计数是否是0，是否可以释放。因为上述方案在临界区因为涉及大量同步操作性能会导致急剧下降，在接近0的时候
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTY4MDA5MTkxNiwtMTU0NTAwMDMzMSw3NT
